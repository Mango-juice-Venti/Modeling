...
+ 업장가중 SMAPE(공식식) 검증 + 블렌딩 최적화 + 업장/메뉴 캘리브레이션
+ 희소아이템 블렌딩 + 7일 재귀예측 + 퍼지매칭 제출

요청 반영사항:
- POS_SAMPLE_BOOST = 4.0
- CAL_CLIP = (0.85, 1.15)  # (보수적 보정)
- SPARSE_NZ_THRESHOLD = 0.10, STALE_DAYS_THRESHOLD = 21
- alphas = np.linspace(0.1, 0.9, 17)

입력:
  C:/Users/minseo/lg/train/train.csv    # 파일명이 re_train.csv면 아래 TRAIN_FILE 수정
  C:/Users/minseo/lg/test/TEST_00_processed.csv ... TEST_09_processed.csv
  C:/Users/minseo/lg/sample_submission.csv
출력:
  C:/Users/minseo/lg/submission_xgb_daily.csv
"""
...
# ====================== Config ======================
BASE_DIR   = Path(r"C:\Users\minseo\lg")   # ← 환경에 맞게 수정
TRAIN_FILE = BASE_DIR / "train" / "re_train.csv"   # 파일명이 re_train.csv면 여기 변경
TEST_GLOB  = str(BASE_DIR / "test" / "TEST_*processed.csv")
SAMPLE_SUB = BASE_DIR / "sample_submission.csv"
OUT_FILE   = BASE_DIR / "submission_5.csv"
RANDOM_STATE = 42
VALID_LAST_DAYS = 28
...
# Tweedie·학습 가중
POS_SAMPLE_BOOST = 4.0   # (요청 반영) y>0 샘플 가중

# 캘리브레이션(검증 합계 비율 보정)
CAL_CLIP = (0.85, 1.15)  # (요청 반영) 보수적 클립
...
# ====================== Calibration ======================
def compute_calibration_from_validation(y_true, y_pred, shops, items):
    df = pd.DataFrame({"y_true": y_true, "y_pred": y_pred, "영업장명": shops, "메뉴": items})
    df = df[df["y_true"] > 0]
    def ratio(a, b):
    ...
    shop_a = df.groupby("영업장명").apply(lambda g: ratio(g["y_true"], g["y_pred"]) if (g["y_true"]>0).sum()>=3 else np.nan).dropna().to_dict()
    item_a = df.groupby("메뉴").apply(lambda g: ratio(g["y_true"], g["y_pred"]) if (g["y_true"]>0).sum()>=3 else np.nan).dropna().to_dict()
    print(f"[CAL] global={global_a:.3f}, shops={len(shop_a)}, items={len(item_a)}")
    return {"global": global_a, "shop": shop_a, "item": item_a}

def apply_calibration_ratio(yhat, item_name, shop_name, cal):
    a = cal["item"].get(item_name)
    if a is None: a = cal["shop"].get(shop_name, cal["global"])
    return float(yhat * a)

# ====================== Train (Dual Models + Blend) ======================
def train_models_and_blend(train_df: pd.DataFrame):
    ...
    # ---- Blend α search on validation (요청 반영: 0.1~0.9, 17분할) ----
    alphas = np.linspace(0.1, 0.9, 17)
    ...
    print(f"[BLEND] best_alpha={best_alpha:.3f} | valid Weighted SMAPE={best_score:.4f}")

    # 최종 calibration(블렌드 예측 기준)
    y_pred_blend_va = (1.0-best_alpha)*predA_va + best_alpha*predB_va
    cal = compute_calibration_from_validation(y_va, y_pred_blend_va, shops_va, items_va)
    ...
    return (modelA, modelB, best_alpha), feats, cat_maps, cal
...
def forecast_7days_for_testfile(models, feats, train_df, test_df, test_tag, cat_maps, cal):
    ...
    for item in items:
        ...
        for k in range(1,8):
            ...
            dmat = xgb.DMatrix(X)
            yA = float(np.clip(modelA.predict(dmat, iteration_range=(0, modelA.best_iteration+1))[0], 0, None))
            yB = float(np.expm1(np.clip(modelB.predict(dmat)[0], 0, None)))
            yhat = (1.0 - alpha) * yA + alpha * yB

            # 캘리브레이션
            yhat = apply_calibration_ratio(yhat, item, upjang, cal)
...
def main():
    ...
    models, feats, cat_maps, cal = train_models_and_blend(train)   # → 블렌딩 α와 valid 점수 출력
    ...
    for tf in test_files:
        ...
        wide = forecast_7days_for_testfile(models, feats, train, test_df, test_tag, cat_maps, cal)
